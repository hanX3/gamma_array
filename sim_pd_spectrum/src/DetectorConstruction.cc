...
G4LogicalVolume *DetectorConstruction::GetGeArrayLog(G4String ge_array_name)
{
  ...
  G4Sphere *solid_ge_array = new G4Sphere("CsI_Array", 0, Ge_ARRAY_R, 0, CLHEP::twopi, 0, CLHEP::pi);
...
G4VPhysicalVolume *DetectorConstruction::DefineVolumes()
{
  ...
  new G4PVPlacement(0, position_ge, ge_log, "Ge", ge_array_log, false, 0, check_overlaps); 

  // Chamber Vaccum
  G4ThreeVector position_chamber_vaccum = G4ThreeVector(0, 0, 0);
  G4LogicalVolume *chamber_vaccum_log = GetChamberVaccumLog("Chamber_Vaccum");
  new G4PVPlacement(0, position_chamber_vaccum, chamber_vaccum_log, "Chamber_Vaccum", logic_world, false, 0, check_overlaps);

  // Chamber
  G4ThreeVector position_chamber = G4ThreeVector(0, 0, 0);
  ...
  // CsI
  G4LogicalVolume *mylar_log = GetMylarLog("Mylar");
  G4LogicalVolume *csi_log = GetCsILog("CsI");
  G4LogicalVolume *si_log = GetSiLog("Si");
  for(int i=0;i<N_OF_CSI_DETECTOR;i++){
    G4ThreeVector position_mylar = G4ThreeVector(CSI_ARRAY_X[i], CSI_ARRAY_Y[i], MYLAR_POS_DIS);
    new G4PVPlacement(0, position_mylar, mylar_log, "Mylar", csi_array_log, false, i, check_overlaps);

    G4ThreeVector position_csi = G4ThreeVector(CSI_ARRAY_X[i], CSI_ARRAY_Y[i], CSI_POS_DIS);
    new G4PVPlacement(0, position_csi, csi_log, "CsI", csi_array_log, false, i, check_overlaps);
  ...
  for(int i=0;i<N_OF_SI_PIN_DETECTOR;i++){
    G4ThreeVector position_si_pin = G4ThreeVector(SI_PIN_ARRAY_X[i], SI_PIN_ARRAY_Y[i], SI_PIN_POS_DIS);
    new G4PVPlacement(0, position_si_pin, si_pin_log, "Si", si_pin_array_log, false, i, check_overlaps);
  }
