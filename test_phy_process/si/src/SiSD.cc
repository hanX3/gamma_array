...
//
SiSD::SiSD(const G4String &sd_name, const G4String &hits_collection_name)
...
void SiSD::Initialize(G4HCofThisEvent *hit_collection)
{
...
G4bool SiSD::ProcessHits(G4Step *step, G4TouchableHistory *)
{
  ...
  SiHit *hit = (*hits_collection)[copy_no];
  hit->SetID(copy_no);
  hit->AddEdep(e); 
